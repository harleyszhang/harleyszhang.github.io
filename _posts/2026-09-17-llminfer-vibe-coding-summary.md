...
**17, 任务：rapid_llm 投机解码模块（dspark 作为草稿来源）模块设计和优化

```bash
# 任务：rapid_llm 新增投机解码模块（dspark/dflash 库作为草稿来源）

## 背景
rapid_llm 新增投机解码能力：草稿 token 由外部库 dspark 生成，
验证与拒绝采样在框架内完成。无损是硬标准。

## 阶段 0：契约确认（此阶段不写实现代码）
以下三项先确认，产出《接口契约》文档后停下来等确认，再动工：
1. 投机解码类型：dspark 提供的是 draft model / Medusa / EAGLE /
   n-gram 中的哪一种；
2. dspark 接口：输入（当前 token 序列？KV？）、输出（k 个候选
   token？是否含草稿概率？）、调用时机、版本约束；
3. 草稿权重谁加载、从哪里来。
契约至少包含：接口签名、错误语义、超时处理。

## 架构设计（核心思想：提议、验证、裁决三者分离）
dspark 只负责"提议"（生成候选），框架负责"验证"（目标模型前向）
和"裁决"（拒绝采样）。职责切开后，换提议者不动框架，改框架不动
提议者。三个结合点各自的设计职责：

1. 与 Scheduler：草稿预算与回退
   - 预算：投机开启时，调度器为每个请求每步预留 k+1 个 token
     槽位（k 个候选 + 1 个 bonus），KV block 分配与 token 记账
     都按此口径；
   - 回退：验证后实际接受 a ≤ k 个，未用槽位回收——token 预算、
     KV block、请求位置游标三方必须一致回滚；
   - 边界：投机只作用于 decode 阶段，prefill chunk 不参与。
2. 与 ModelRunner：单次批量验证
   - k 个候选拼成一次前向（每请求长度 k+1），一次 forward 拿到
     k+1 个位置的 logits，禁止 k 次串行验证；
   - 候选位置的 attention mask 按链式处理（树形候选本期不做）；
   - batch 内各请求实际 k 不同，给出 padding 或变长的处理策略。
3. 与 Sampler：拒绝采样与 bonus token
   - 逐位置裁决：位置 i 接受则用草稿 token；第一个被拒位置从
     修正分布采样；
   - 全部接受时，从目标模型第 k+1 个分布采 bonus token——
     bonus 是加速收益的一部分，漏掉等于白丢一个 token；
   - RNG 状态管理见耦合点 4。

## 范围（做什么）
1. 模块结构：
   - spec_decode/：提议者按方法拆分独立模块，dspark 接入是其一；
   - sample/rejection_sampler.py：拒绝采样器，无损的关键；
   - spec_decode/spec_decode_worker.py：主协调器。
2. 配置项：投机开关、每步候选数 k、自适应关闭阈值（接受率低于
   阈值自动关闭投机）。
3. dspark 接入层：薄封装；库不可用时明确报错，禁止静默退化。

## 范围（不做什么）
- 不训练、不制作草稿模型；
- 不实现树形候选（chain-only）；
- 不改变已有调度特性的语义（chunked prefill、prefix caching
  保持原行为）。

## 四个高风险耦合点（逐项设计说明 + 测试）
1. KV cache 回滚：被拒 token 的 KV 槽位正确回收；测试覆盖
   全部接受 / 部分接受 / 全部拒绝三条路径。
2. batch 状态分叉：同一步各请求接受数不同、batch 中途变化时，
   调度器与 runner 的 token 预算记账一致。
...
1. 无损性：
   ...
   - 采样：同种子逐 token 一致（首选）；做不到则分布检验：
     同温度各生成 ≥10 万 token，卡方检验 p > 0.05，报告方法
     与原始数据。
2. 加速（负载固定写明，接受率是负载的函数）：
   - 负载 A（高接受率，如代码续写）：TPOT/吞吐为正收益；
   - 负载 B（低接受率，如高温度开放生成）：劣化 ≤ 5%，
     自适应开关触发正确。
3. 四耦合点测试全覆盖，含接受率 0% / 100% 边界。
4. 交叉矩阵逐格实测（每格 = greedy 一致 + 性能无回归）：
   投机 × {CUDA Graph、TP、量化、通信-计算重叠、
   chunked prefill、prefix caching}；负叠加记录原因。
5. 稳定性：高并发压测 ≥ 30 分钟，无状态错乱，显存曲线平稳。
6. dspark 隔离：单测全 mock；接口变更能被测试立刻暴露。

## 交付物
1. 《接口契约》文档（阶段 0 产出）；
2. 架构设计说明：三分离思想 + 三个结合点 + 四耦合点；
3. 测试与 benchmark 报告：环境、负载、可复现命令、原始日志、
   指标（TTFT、TPOT、TPS、TGS、吞吐、延迟分位、错误率、显存、
   GPU 利用率、接受率、每步平均接受 token 数）、投机开/关与
   不同接受率负载的对比结论；
4. README 更新：原理、开关与调参；gif 为终端实录的
   投机开/关速率对比；
5. git：新分支，commit 拆分——投机解码核心 / dspark 接入层 /
   runner 集成 / 交叉验证与文档，每个 commit 独立过测试，
   完成后推送远程。
```
