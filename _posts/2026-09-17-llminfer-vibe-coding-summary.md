...
- [一 框架详解/文档优化提示词集合](#一-框架详解文档优化提示词集合)
- [二 自研推理服务框架开发提示词集合](#二-自研推理服务框架开发提示词集合)
  - [2.1 更新优化 RELEASE 和 README 文档](#21-更新优化-release-和-readme-文档)
  - [2.2 优化模块/组件的代码质量及可读性治理](#22-优化模块组件的代码质量及可读性治理)
  - [2.3 设计实现框架的 Benchmark 模块](#23-设计实现框架的-benchmark-模块)
  - [2.4 量化模型的 benchmark 测试](#24-量化模型的-benchmark-测试)
  - [2.5 验证裁剪后模型的推理正确性](#25-验证裁剪后模型的推理正确性)
  - [2.6 全面深度重构框架和及模型性能优化](#26-全面深度重构框架和及模型性能优化)
  - [2.7 框架的默认精度策略改为 auto](#27-框架的默认精度策略改为-auto)
  - [2.8 模块/组件的代码质量治理](#28-模块组件的代码质量治理)
  - [2.9 重构框架的模型配置/权重加载/模型注册模块](#29-重构框架的模型配置权重加载模型注册模块)
  - [2.10 新增框架的分布式并行推理模块](#210-新增框架的分布式并行推理模块)
  - [2.11 新增开发框架的量化特性](#211-新增开发框架的量化特性)
  - [2.12 框架新增模型支持](#212-框架新增模型支持)
  - [2.13 框架已支持模型的性能优化](#213-框架已支持模型的性能优化)
  - [2.14 框架的算子模块架构设计与新算子实现](#214-框架的算子模块架构设计与新算子实现)
  - [2.15 优化框架的不同粒度的 overlap](#215-优化框架的不同粒度的-overlap)
  - [2.16 优化框架的服务调度能力](#216-优化框架的服务调度能力)
  - [2.17 框架新增投机解码模块](#217-框架新增投机解码模块)
  - [2.18 重构框架的模型注册加载机制](#218-重构框架的模型注册加载机制)
  - [2.19 重构框架已开发组件](#219-重构框架已开发组件)
- [三 AI 代码治理和编程经验](#三-ai-代码治理和编程经验)
  - [3.1 优先建立 AI 编码准则](#31-优先建立-ai-编码准则)
  - [3.2 测试先行，再开发特性](#32-测试先行再开发特性)
  - [3.3 AI 代码的 Code Review 重点转移](#33-ai-代码的-code-review-重点转移)
  - [3.4 按模型能力对 AI 生成代码分级管理](#34-按模型能力对-ai-生成代码分级管理)
  - [3.5 必须有人真正理解框架代码的原理、设计与算法](#35-必须有人真正理解框架代码的原理设计与算法)
...
**5，任务：裁剪层数后的模型如何真实推理并做性能 benchmark**：

```bash
任务：超大模型的裁剪验证（显存受限下的正确性与引擎对比）

## 背景
全尺寸模型（如 671B 级）放不进现有 GPU 显存。通过裁剪层数得到小尺寸
checkpoint，用于：验证 rapid_llm 对该模型的支持正确性，并与参考引擎
（vLLM / SGLang / transformers）在完全相同的小模型上做精度与性能对比。
注意：裁剪模型的性能数字只用于引擎间横向对比，不代表全尺寸模型性能。

## 范围（做什么）
1. 裁剪脚本：输入（源权重目录、保留层数 N、输出目录），产出新
   checkpoint：embedding + 前 N 层 + final norm + lm_head；
   同步修改 config（num_hidden_layers=N），其余字段不动。
2. 层数 N 的选择规则：保留层必须覆盖模型用到的全部算子类型。
   已知陷阱：部分模型前几层是 dense、之后才出现 MoE（如 DeepSeek
   前 3 层无专家），N 必须大过 dense 前缀；MTP 等附属模块是否保留，
   明确写出决定与理由。
3. 权重加载验证：真实 config、真实权重（路径 /path/llm_weights/，
   自行搜索或向用户确认）。加载报告必须输出：missing keys = 0、
   unexpected keys = 0。禁止随机初始化充数。
4. 算子覆盖验证：列出该模型的算子清单（attention 变体、MoE 路由、
   norm、RoPE、量化 kernel 等），用 profiler/kernel trace 证明
   每个算子在 e2e 推理中至少执行一次。
5. 精度对比：同一裁剪 checkpoint 分别在 rapid_llm 与参考引擎加载，
   同一组 prompt、贪心解码，逐 token 对比。
6. 性能对比：同一硬件、同一负载，两引擎各测 TTFT / TPOT / 吞吐 /
   显存。

## 范围（不做什么）
- 不声称裁剪模型的数字代表全尺寸性能；
- 不修权重、不微调；
- 不为通过精度对比而放宽容差——有差异先查原因。

## 验收标准
1. 加载报告：missing/unexpected keys 均为零（随机初始化直接判失败）。
2. e2e 推理输出连贯文本。
3. 算子覆盖：清单中每个算子有执行证据。
4. 精度：与参考引擎贪心输出逐 token 一致率 ≥ 99%，
   不一致的样本逐个给出分析。
5. 性能：两引擎同条件数据齐全，结论含差异解释。

## 交付物（按 Benchmark 模块工作标准）
1. 环境：rapid_llm 版本/commit、CUDA/驱动、硬件、模型与裁剪层数 N、
   参考引擎版本、启动参数；
2. 负载：prompt 集、输入/输出长度、并发、采样参数；
3. 完整可复现命令（含裁剪脚本的用法）；
4. 原始或关键日志（含权重加载报告、算子覆盖证据）；
5. 指标：TTFT、TPOT、TPS、TGS（有并行时）、吞吐、延迟分位、
   错误率、显存/GPU 利用率；
6. 对比总结：rapid_llm vs 参考引擎，结论与异常说明。
```

### 2.6 全面深度重构框架和及模型性能优化

**6，任务：对 rapid_llm 框架进行全面重构与性能优化**：

```bash
任务：对 rapid_llm 框架进行全面重构与性能优化。严格分阶段执行，每阶段独立验收。
