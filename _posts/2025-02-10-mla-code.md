...
<div align="center">
<img src="../images/deepseekv2/transformers-mla-mermaid-diagram2.jpg" width="100%" alt="transformers-mla-mermaid-diagram">
</div>
...
flowchart TD
  ...
  %% 样式定义
  %% =========================
  classDef inputOutput fill:#f0f8ff,stroke:#1e90ff,stroke-width:2px,color:#000,font-weight:bold;
  classDef downProj fill:#e8f5e9,stroke:#4caf50,stroke-width:2px;
  classDef normNode fill:#d1c4e9,stroke:#673ab7,stroke-width:2px,color:#000,font-weight;
  classDef upProj fill:#fff3e0,stroke:#ff9800,stroke-width:2px;
  classDef splitNode fill:#fafafa,stroke:#9e9e9e,stroke-width:1px,stroke-dasharray: 5 5;
  classDef rope fill:#fce4ec,stroke:#e91e63,stroke-width:2px,font-weight:bold;
  classDef attnBlock fill:#e3f2fd,stroke:#2196f3,stroke-width:3px;
  classDef cache fill:#fffde7,stroke:#f9a825,stroke-width:2px;

  %% =========================
  %% 0. 输入
  %% =========================
  Start["输入: hidden_states<br/>(B, S, H)"]

  %% =========================
  %% 1. Q 分支 (Down -> Norm -> Up)
  %% =========================
  Start --> QDown["Q Down Projection<br/>q_a_proj"]
  QDown --> QNorm["Q RMSNorm<br/>q_down_rmsnorm"]
  QNorm --> QUp["Q Up Projection<br/>q_b_proj"]
  
  %% Q Split: Up 之后分离 Pe (NoPE) 和 RoPE
  QUp --> SplitQ{"Split Q"}
  SplitQ --> QNope["Q NoPE (Content)<br/>(B, S, nH, d_nope)"]
  SplitQ --> QRotRaw["Q RoPE Input<br/>(B, S, nH, d_rope)"]

  %% =========================
  %% 2. KV 分支 (Fused Down -> Split)
  %% =========================
  %% 关键修正：kv_down_proj 同时产生 Latent KV 和 K_RoPE
  Start --> KVDown["KV Fused Down Projection<br/>kv_a_proj_with_mqa"]
  
  %% Split 发生在 Norm 之前
  KVDown --> SplitKVDown{"Split kv_down"}
  
  %% 路径 A: Latent KV -> Norm -> Up (生成 K_NoPE 和 V)
  SplitKVDown -->|Latent KV| KVNorm["KV RMSNorm<br/>kv_down_rmsnorm"]
  KVNorm --> KVUp["KV Up Projection<br/>kv_b_proj"]

  %% KV Up 输出需要再次 Split 为 K_NoPE 和 V
  KVUp --> SplitKVUp{"Split kv_up"}
  SplitKVUp --> KNope["K NoPE (Content)<br/>(B, S, nH, d_nope)"]
  SplitKVUp --> Val["Value (V)<br/>(B, S, nH, d_v)"]

  %% 路径 B: K_RoPE (直接旁路，不经过 Latent Norm/Up)
  SplitKVDown -->|K_RoPE Part| KRotRaw["K RoPE Input<br/>(B, S, nH, d_rope)"]

  %% =========================
  %% 4. 最终合并 (Concat)
  %% =========================
  subgraph Concat_Logic ["构建 Head 向量"]
    direction TB
    %% =========================
    %% 3. RoPE 计算
    %% =========================
    QRotRaw --> ApplyRoPE["apply_rotary_pos_emb\n(q_rope, k_rope, cos, sin, position_ids)"]
    KRotRaw --> ApplyRoPE

    ApplyRoPE --> QRot["Q RoPE (Rotated)"]
    ApplyRoPE --> KRot["K RoPE (Rotated)"]

    QNope --> MergeQ["Concat Final Q<br/>[Q_nope; Q_rope]"]
    QRot --> MergeQ
    
    KRot --> MergeK
    KNope --> MergeK["Concat Final K<br/>[K_nope; K_rope]"]

  end

  %% =========================
  %% 5. Self-Attention 核心 (框出)
  %% =========================
  subgraph Attn_Core ["FlashAttention / PagedAttention"]
    direction TB
    %% 缓存处理
    MergeK --> KCache["KV Cache (Key)"]
    Val --> VCache["KV Cache (Value)"]

    %% 计算逻辑
    MergeQ --> AttnOp
    KCache --> AttnOp["Score = Q @ K^T<br/>Softmax(Score)"]
    AttnOp --> AttnWeighted["Output = Prob @ V"]
    VCache --> AttnWeighted
  ...
  %% 6. 输出
  %% =========================
  AttnWeighted --> OProj["Output Projection<br/>o_proj"]
  OProj --> End["输出: attn_output"]

  %% =========================
  %% 样式应用
  %% =========================
  class Start,End inputOutput;
  class QDown,KVDown downProj;
  class QNorm,KVNorm normNode;
  class QUp,KVUp,OProj upProj;
  class SplitQ,SplitKVDown,SplitKVUp,QNope,KNope,Val,QRotRaw,KRotRaw,QRot,KRot splitNode;
  class ApplyRoPE_Q,ApplyRoPE_K rope;
  class Attn_Core attnBlock;
  class KCache,VCache cache;
```
