...
<div align="center">
<img src="../images/deepseekv2/transformers-mla-mermaid-diagram.svg" width="70%" alt="transformers-mla-mermaid-diagram">
</div>
...
graph TD
    ...
    %% 样式定义
    %% ==========================================
    classDef inputOutput fill:#f0f8ff,stroke:#1e90ff,stroke-width:3px,color:#000,font-weight:bold;
    classDef downProj fill:#e8f5e9,stroke:#4caf50,stroke-width:2px;
    classDef normNode fill:#d1c4e9,stroke:#673ab7,stroke-width:2px,color:#000,font-weight:bold;
    classDef upProj fill:#fff3e0,stroke:#ff9800,stroke-width:2px;
    classDef rope fill:#fce4ec,stroke:#e91e63,stroke-width:2px;
    classDef attention fill:#e3f2fd,stroke:#2196f3,stroke-width:2px;
    classDef merge fill:#f3e5f5,stroke:#9c27b0,stroke-width:2px;

    %% ==========================================
    %% 1. 输入与 Down Projection (压缩)
    %% ==========================================
    Start[输入: hidden_states<br/>(batch, seq_len, hidden_size)] --> Q_Down[Q Down Projection<br/>q_a_proj<br/>(压缩为 Latent Q)]
    Start --> KV_Down[KV Down Projection<br/>kv_a_proj_with_mqa<br/>(压缩为 Latent KV)]

    %% ==========================================
    %% 2. RMSNorm (新增核心部分)
    %% ==========================================
    Q_Down --> Q_Norm[Q RMSNorm<br/>q_a_layernorm]
    KV_Down --> KV_Norm[KV RMSNorm<br/>kv_a_layernorm]

    %% ==========================================
    %% 3. Up Projection (升维/解压)
    %% ==========================================
    Q_Norm --> Q_Up[Q Up Projection<br/>q_b_proj]
    KV_Norm --> KV_Up[KV Up Projection<br/>kv_b_proj]

    %% ==========================================
    %% 4. 分离基础部分与旋转部分 (Split)
    %% ==========================================
    %% Query 流
    Q_Up --> SplitQ_Base[分离: Q Pe (基础部分)<br/>(batch, seq, heads, qk_nope_head_dim)]
    Q_Up --> SplitQ_Rot[分离: Q RoPE (旋转部分)<br/>(batch, seq, heads, qk_rope_head_dim)]

    %% Key-Value 流
    KV_Up --> SplitK_Base[分离: K Pe (基础部分)<br/>(batch, seq, heads, qk_nope_head_dim)]
    KV_Up --> V[分离: Value<br/>(batch, seq, heads, v_head_dim)]
    
    %% 注意：DeepSeek V2/V3 代码中，K 的 RoPE 部分有时是通过独立的 k_rope 投影或复用逻辑
    %% 这里展示最常见的解耦后 K_Rot 处理逻辑
    Start -.->|或者独立路径| SplitK_Rot[K RoPE 部分<br/>(通常来自 kv_a_proj 或独立计算)]
    KV_Up -.->|部分实现包含在Up中| SplitK_Rot

    %% ==========================================
    %% 5. 应用 RoPE
    %% ==========================================
    SplitQ_Rot --> RoPE[旋转位置编码 RoPE]
    SplitK_Rot --> RoPE

    %% ==========================================
    %% 6. 合并 (Concat)
    %% ==========================================
    SplitQ_Base --> MergeQ[Concat: Final Q]
    RoPE --> MergeQ
    
    SplitK_Base --> MergeK[Concat: Final K]
    RoPE --> MergeK

    %% ==========================================
    %% 7. 注意力计算
    %% ==========================================
    MergeQ --> Attn[注意力得分<br/>Q @ K^T]
    MergeK --> Attn
    Attn --> Softmax[Softmax]
    V --> AttnOut[加权求和<br/>Score @ V]
    Softmax --> AttnOut

    %% ==========================================
    %% 8. 输出
    %% ==========================================
    AttnOut --> OProj[输出投影<br/>o_proj]
    OProj --> End[输出: attn_output]

    %% ==========================================
    %% 子图分组
    %% ==========================================
    subgraph MLA_Compression ["MLA 压缩与归一化"]
        Q_Down
        KV_Down
        Q_Norm
        KV_Norm
    ...
    subgraph MLA_Up ["MLA 升维"]
        Q_Up
        KV_Up
    end

    subgraph Core_Attn ["Flash/Paged Attention"]
        Attn
        Softmax
        AttnOut
    end
    ...
    class Q_Down,KV_Down downProj;
    class Q_Norm,KV_Norm normNode;
    class Q_Up,KV_Up upProj;
    class RoPE rope;
    class Attn,Softmax,AttnOut attention;
    class MergeQ,MergeK merge;
```
