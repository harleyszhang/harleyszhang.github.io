...
<div align="center">
<img src="../images/deepseekv2/transformers-mla-mermaid-diagram.jpg" width="100%" alt="transformers-mla-mermaid-diagram">
</div>
...
flowchart TD
  ...
  %% =========================
  %% 1. Q 分支 (Down -> Norm -> Up -> Split)
  %% =========================
  Start --> QDown["Q Down Projection<br/>q_a_proj"]
  QDown --> QNorm["Q Down RMSNorm<br/>q_a_rmsnorm"]
  ...
  %% =========================
  %% 2. KV 分支 (Fused Down -> Split -> Norm -> Up -> Split)
  %% =========================
  %% Step 1: Fused Down
  Start --> KVDown["KV Fused Down Projection<br/>kv_a_proj_with_mqa"]
  
  %% Step 2: Split Down Output (分离 Latent KV 和 K_RoPE)
  KVDown --> SplitKVDown{"Split kv_down"}
  
  %% 路径 A: Latent KV -> Norm -> Up -> (K_NoPE + V)
  SplitKVDown -->|Latent KV| KVNorm["KV Down RMSNorm<br/>kv_a_rmsnorm"]
