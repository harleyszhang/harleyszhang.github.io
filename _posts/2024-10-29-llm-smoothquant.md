...
$$Y = (X \cdot \text{diag}(\mathrm {s} )^{-1}) \cdot (\text{diag}(\mathrm {s}) \cdot W) = \hat{X} \cdot \hat{W} \tag{3}$$
...
- 一种极端情况，是令 $s_j = \max(\left\| \mathbf{X}_j \right\|), \quad j = 1, 2, \dots, C_i$，这会使得激活值的离群值全部转移到权重上面，激活值变得很好量化，权重反而难以量化。
- 另一种极端情况，是令 $s_j = \frac{1}{\max(\left\| \mathbf{W}_j \right\|)}$，即**将所有量化难度从权重迁移到激活**，这会使得原本方差较小的权重方差更小，激活值方差更大。
