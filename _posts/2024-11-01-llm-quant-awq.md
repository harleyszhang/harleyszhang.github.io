...
- [2. AWQ: 激活感知的权重量化](#2-awq-激活感知的权重量化)
  - [2.1 观点 1-保留 `1%` 的关键权重来提升 LLM 量化性能](#21-观点-1-保留-1-的关键权重来提升-llm-量化性能)
  - [2.2 观点 2-基于激活感知缩放保护关键权重](#22-观点-2-基于激活感知缩放保护关键权重)
...
### 2.1 观点 1-保留 `1%` 的关键权重来提升 LLM 量化性能
...
> 到这里，论文就描述完了观点 2 的由来，是通过公式推导和实验证明。
...
先看直接结果即模型精度变化情况，结果显示，放大显著通道非常有效：模型困惑度 PPL 从 $s = 1$ 时的 23.54（仅为 RTN）降低到 $s = 2$ 时的 11.92。
...
如何选取 $s$ 来平衡显著通道和非显著通道的误差，就是下一节内容了，也是这篇论文相对前作的核心贡献，毕竟权重乘以缩放因子的方法 `smoothquant` 论文已经提出了，**这节更多的是通过公式推导和实验证明了权重乘以 $> 1$ 的缩放因子可以降低（权重）量化误差**！而前作 `smoothquant` 针对权重乘以缩放因子 $s$ 就可以将激活量化难度转移至权重（实际就是降低量化误差）并没有给出一个很详细的理论说明和公式推导。此外，权重缩放因子 $s$ 的计算方法还是比较粗糙的，因此，awq 论文进一步优化了 $s$ 的计算法。
