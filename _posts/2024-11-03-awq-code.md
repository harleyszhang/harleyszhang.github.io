...
def pack_intweight(unpacked_qweight, interleave, kstride):
    """
    对量化的权重矩阵进行压缩和重新排列，以优化存储和后续操作的性能. 
    参数:
        - unpacked_qweight：输入的量化权重张量，形状为 [N, K]
        - interleave：控制行的交错排列的步长，决定每多少行进行一次交错操作，以提高数据的访问效率
        - kstride：控制列的步长，用于重新排列列，以便在去量化过程中以分块的方式进行访问。
    """
    # 1. 基本信息提取：从 unpacked_qweight 获取形状 N 和 K，这是压缩前权重的行数和列数。
    N = unpacked_qweight.shape[0]
    K = unpacked_qweight.shape[1]

    # 2. 将权重调整为 [N, K//32, 32] 的形状，以便于按 32 个元素进行处理。
    Packed_Kernel = unpacked_qweight.cpu().numpy().reshape(N, K // 32, 32)

    # 3. 重新排列：通过 .reshape 和 .transpose，对权重进行分组并调整维度，以优化数据布局，使之更适合批量化操作和去量化。
    Packed_Kernel = Packed_Kernel.reshape(N, K // 32, 4, 4, 2).transpose(0, 1, 3, 2, 4)
    Packed_Kernel = Packed_Kernel.reshape(N, K // 32, 32)
    Packed_Kernel = Packed_Kernel.reshape(N, K // 32, 4, 8)
    Packed_Kernel = Packed_Kernel.reshape(N, K // 32, 4, 4, 2).transpose(0, 1, 2, 4, 3)
    Packed_Kernel = Packed_Kernel.reshape(N, K) # 重塑回 [N, K]

    # 4.调整形状并交错排列，以便压缩权重，使数据更高效地利用缓存。重塑为 [N//interleave, interleave, K//kstride, kstride]
    Packed_Kernel = Packed_Kernel.reshape(N // interleave, interleave, K // kstride, kstride)
    # 转置为 [N//interleave, K//kstride, interleave, kstride]
    Packed_Kernel = Packed_Kernel.transpose(0, 2, 1, 3)
    # 重塑为 [N//interleave, K//kstride, kstride, interleave]
    Packed_Kernel = Packed_Kernel.reshape(
        N // interleave, K // kstride, kstride, interleave
    )

    # 5. 每四个权重打包为一个 int16，左移位并按位或操作, 并 重塑为 [N//interleave, K]
    Packed_Kernel = (
        Packed_Kernel[..., 0]
        | (Packed_Kernel[..., 1] << 4)
        | (Packed_Kernel[..., 2] << 8)
    ...
    # 6. 转换为 int16 类型的张量，并移动到原设备后，输出权重张量
    qweight = (
