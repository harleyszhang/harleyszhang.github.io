...
之所以需要 `casual masked` 是因为，**对于因果模型来说，在 `prefill` 阶段只需要关注当前 token 与之前 tokens 的注意力关系，而无需理会它与后续 tokens 的关系**，基于此这种模型的 mask 被称为 `Causal Mask`。
...
上图可以清晰看出，如果想要实现让当前 `token` 只关注（看）它及之前位置的 `tokens` 的注意力关系，关键是**构建 `Casual Mask`: 尺寸为`[seq_len, seq_len]` 的下三角矩阵**。
