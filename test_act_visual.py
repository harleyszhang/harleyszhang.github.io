...
# 运行推理并获取激活值和权重
@torch.no_grad()
def get_activations_and_weights(model, tokenizer, texts, layer_index = 4, channel_indexs = 200):
    inputs = tokenizer(texts, return_tensors="pt", padding=True, truncation=True).cuda()
    with torch.no_grad():
...
# 主函数，执行各个步骤
def main():
    model_name = "/home/honggao/llm_weights/Qwen3-1.7B"
    ...
    activation_sample, weight_sample = get_activations_and_weights(model, tokenizer, input_texts, layer_index = 4,channel_indexs=200)
